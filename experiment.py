...
def run_single_experiment(config):
    ...
    (rician_map, gaussian_map) = main.rice_homomorf_est(noisy, snr, config['lpf_f'], config['ex_filter_type'], config=config);
...
def run_example_experiments(config):
    ...
    now = clock()
    (rician_map, gaussian_map) = main.rice_homomorf_est(input_noisy, input_snr, lpf, 2);
    ...
    now = clock()
    (rician_map, gaussian_map) = main.rice_homomorf_est(input_noisy, array([0]), lpf, 2);
    ...
    now = clock()
    (rician_map, gaussian_map) = main.rice_homomorf_est(input_noisy, input_snr, lpf, 1);
    ...
    now = clock()
    (rician_map, gaussian_map) = main.rice_homomorf_est(input_noisy, array([0]), lpf, 1);
